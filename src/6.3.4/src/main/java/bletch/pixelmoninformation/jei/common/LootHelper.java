    	...
    	try {
    		...
    		if (value instanceof PokemonDropInformation) {
    			...
    			pokemonDrops.entrySet().stream()
    					.filter(d -> d.getKey() instanceof EnumPokemon)
    					.map(d -> new PokemonDrop((EnumPokemon)d.getKey(), sequence[0], (PokemonDropInformation)d.getValue()))
    					.forEach(drops::add);
    		}
    			...
    			pokemonDrops.entrySet().stream()
						...
						.forEach(d -> {
							...
							((Set<?>)d.getValue()).stream()
									...
									.map(i -> new PokemonDrop((EnumPokemon)d.getKey(), sequence[0]++, (PokemonDropInformation)i))
									.forEach(drops::add);
						});
    		}
