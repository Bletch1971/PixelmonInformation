...
public class PokeChestWrapper implements IRecipeWrapper, ITooltipCallback<ItemStack> {

	private static final int buttonWidth = 13;
	private static final int buttonHeight = 13;
	
	private final PokeChestEntry entry;
	private IFocus<ItemStack> focus;
    private GuiButton buttonPrev;
    private GuiButton buttonNext;
    private int pageIndex;
    
    public PokeChestWrapper(PokeChestEntry entry) {
        this.entry = entry;
        this.focus = null;
        this.pageIndex = 1;
		...
		if (guiHelper != null) {
			...
			if (arrowPrevious != null && arrowNext != null) {
				this.buttonPrev = new GuiIconButtonSmall(0, 3, 122, buttonWidth, buttonHeight, arrowPrevious);
	        	this.buttonNext = new GuiIconButtonSmall(1, 153, 122, buttonWidth, buttonHeight, arrowNext);
	        	updateButtonsState();
			}
        }
    }

	public PokeChestEntry getEntry() {
		return this.entry;
	}
	...
	public void getIngredients(IIngredients ingredients) {
		ingredients.setInput(VanillaTypes.ITEM, this.entry.getInput());
    ...
    public List<ItemStack> getItems(@Nullable IFocus<ItemStack> focus) {
    	// check if the focus matches the input item
    	if (focus != null && ItemStack.areItemsEqual(focus.getValue(), entry.getInput())) {
    		// focus does match, reset the focus
    		focus = null;
    	}
    ...
    public void updateButtonsState() {
    	double temp = (double)amountOfItems() / (double)PokeChestCategory.ITEMS_PER_PAGE;
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
    	Font.normal.printLeft(this.entry.getInput().getDisplayName(), 30, 5);
        Font.small.printLeft(this.entry.getLocalisedName(), 30, 15);
    	...
    	if (pages > 1) {
    		...
    		Gui.drawRect(3, 122, 166, 135, 0x40FFFFFF);
    		Font.normal.printCentered(this.pageIndex + "/" + pages, 84, 125, Color.WHITE.getRGB(), true);
    	}
